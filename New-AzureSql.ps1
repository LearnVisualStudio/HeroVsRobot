<# 
    ...
    Specifies a name for the application database. 
    ...
    .EXAMPLE 
    .\New-AzureSql.ps1 -DatabaseName TestApp ` 
...
{ 
    ...
    $s = Get-AzureSubscription -ExtendedDetails -Current 
...
{ 
    ...
    Return "Server=tcp:$DatabaseServerName.database.windows.net,1433;Database=$ApplicationDatabaseName;User ID=$UserName@$DatabaseServerName;Password=$Password;Trusted_Connection=False;Encrypt=True;Connection Timeout=30;" 
} 
...
Write-Verbose "[Start] creating database  $ApplicationDatabaseName in database server $databaseServerName" 
$appdb = New-AzureSqlDatabase -DatabaseName $ApplicationDatabaseName -Context $context -Verbose 
if (!$appdb) {throw "Failed to create $ApplicationDatabaseName application database. Failure in New-AzureSqlDatabase in New-AzureSql.ps1"} 
Write-Verbose "[Finish] creating database $ApplicationDatabaseName in database server $databaseServerName" 

Write-Verbose "Creating database connection string for $ApplicationDatabaseName in database server $databaseServerName" 
$appDatabaseConnectionString = Get-SQLAzureDatabaseConnectionString -DatabaseServerName $databaseServerName -DatabaseName $ApplicationDatabaseName -UserName $UserName -Password $Password 
if (!$appDatabaseConnectionString) {throw "Failed to create application database connection string for $ApplicationDatabaseName. Failure in Get-SQLAzureDatabaseConnectionString function in New-AzureSql.ps1"} 
 
Write-Verbose "Setting MyDatabaseName Variable..."
$MyDatabaseName = $ApplicationDatabaseName

Write-Verbose "Creating hash table to return..." 
Return @{ Server = $databaseServerName; UserName = $UserName; Password = $Password; AppDatabase = @{Name = $MyDatabaseName; ConnectionString = $appDatabaseConnectionString}; } 
